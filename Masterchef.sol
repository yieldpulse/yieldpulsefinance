...
contract YieldxToken is ERC20('Yieldx.Finance', 'YIELDX') {
    ...
    */
        ...
        external
    {
        ...
        require(block.timestamp <= expiry, "YIELDX::delegateBySig: signature expired");
...
// MasterChef is the master of YieldxToken. He can make YieldxToken and he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    uint256 public yieldxPerBlock;
    // Bonus muliplier for early yieldx makers.
    ...
    constructor(
        ...
        uint256 _yieldxPerBlock,
        uint256 _startBlock
    ) {
        yieldx = _yieldx;
        devaddr = _devaddr;
        feeAddress = _feeAddress1;
        yieldxPerBlock = _yieldxPerBlock;
    ...
    // Update the given pool's YIELDX allocation point and deposit fee. Can only be called by the owner.
    ...
    function pendingYieldx(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 yieldxReward = multiplier.mul(yieldxPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    // Deposit LP tokens to MasterChef for YIELDX allocation.
    ...
    function safeYieldxTransfer(address _to, uint256 _amount) internal {
        uint256 yieldxBal = yieldx.balanceOf(address(this));
        if (_amount > yieldxBal) {
            yieldx.transfer(_to, yieldxBal);
        } else {
    ...
    function updateEmissionRate(uint256 _yieldxPerBlock) public onlyOwner {
        massUpdatePools();
        yieldxPerBlock = _yieldxPerBlock;
    }
}
